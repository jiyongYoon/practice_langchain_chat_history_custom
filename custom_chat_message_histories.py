...
def _create_table_and_index(table_name: str) -> List[sql.Composed]:
    ...
    index_name = f"idx_{table_name}_user_email_session_id"
    statements = [
        sql.SQL(
            """
        ...
        sql.SQL(
            """
            CREATE INDEX IF NOT EXISTS {index_name} ON {table_name} (user_email, session_id);
            """
        ).format(
...
def _get_messages_query(table_name: str, window_size: int) -> sql.Composed:
    ...
    return sql.SQL("""
        ...
        limit=sql.SQL("LIMIT %(limit)s") if window_size is not None else sql.SQL("")
    )
...
def _delete_by_session_id_query(table_name: str) -> sql.Composed:
    ...
    return sql.SQL(
        "DELETE FROM {table_name} WHERE user_email = %(user_email)s AND session_id = %(session_id)s;"
    ).format(table_name=sql.Identifier(table_name))
...
def _insert_message_query(table_name: str) -> sql.Composed:
    ...
    return sql.SQL(
        "INSERT INTO {table_name} (user_email, session_id, message) VALUES (%s, %s, %s)"
    ).format(table_name=sql.Identifier(table_name))


class MyPostgresChatMessageHistory(BaseChatMessageHistory):
    def __init__(
        self,
        table_name: str,
        user_email: str,
        ...
        # self._aconnection = initialize_async_connection()
        ...
        if self._connection:
            self.create_tables(self._connection, self._table_name)

    async def initialize_async_connection(self):
        if self._aconnection is None:
            self._aconnection = await db.initialize_async_connection()
    ...
    def add_messages(self, messages: Sequence[BaseMessage]) -> None:
        ...
        values = [
            (self._user_email, self._session_id, json.dumps(message_to_dict(message)))
    ...
    def get_messages(self) -> List[BaseMessage]:
        ...
        query = _get_messages_query(self._table_name, self._window_size)
        ...
        with self._connection.cursor() as cursor:
            ...
            if self._window_size is not None:
                params["limit"] = self._window_size * 2
    ...
    def clear(self) -> None:
        ...
        with self._connection.cursor() as cursor:
            cursor.execute(query, {"user_email": self._user_email, "session_id": self._session_id})
        self._connection.commit()
    ...
    async def aclear(self) -> None:
        ...
        query = _delete_by_session_id_query(self._table_name)
        async with self._aconnection.cursor() as cursor:
            await cursor.execute(query, {"user_email": self._user_email, "session_id": self._session_id})
        await self._aconnection.commit()
